...
export async function POST(
  ...
  if (!rateLimit(request, "auth", 10))
    return NextResponse.json(
      { error: "محاولات كثيرة. انتظر دقيقة ثم حاول مجددًا." },
      { status: 429 },
    );
  ...
  try {
    ...
    if (action === "register") {
      ...
      if (!parsed.success)
        return NextResponse.json(
          {
            error:
              "راجع البيانات: جميع الحقول مطلوبة، ورقم الجوال والبريد يجب أن يكونا صحيحين، وكلمة المرور ٨ أحرف على الأقل.",
          },
          { status: 400 },
        );
      ...
      if (exists)
        return NextResponse.json(
          { error: "هذا البريد مسجل بالفعل. يمكنك تسجيل الدخول إلى حسابك." },
          { status: 409 },
        );
    ...
    if (action === "login") {
      ...
      if (!parsed.success)
        return NextResponse.json(
          { error: "أدخل بريدًا إلكترونيًا وكلمة مرور صحيحة." },
          { status: 400 },
        );
      const [company] = await db
        .select()
        .from(companies)
        .where(eq(companies.email, parsed.data.email))
        .limit(1);
      if (
        !company ||
        !(await verifyPassword(parsed.data.password, company.passwordHash))
      )
        return NextResponse.json(
    ...
    return NextResponse.json(
      { error: "تعذر إتمام العملية الآن. حاول مرة أخرى بعد قليل." },
      { status: 500 },
    );
  }
}
