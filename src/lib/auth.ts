...
export async function createSession(companyId: string) {
  ...
  await db.insert(sessions).values({ tokenHash: tokenHash(token), companyId, expiresAt });
  (await cookies()).set(cookieName, token, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/", expires: expiresAt });
}
export async function getCompany() {
  const token = (await cookies()).get(cookieName)?.value;
  if (!token) return null;
  const [result] = await db.select({ company: companies }).from(sessions).innerJoin(companies, eq(sessions.companyId, companies.id)).where(and(eq(sessions.tokenHash, tokenHash(token)), gt(sessions.expiresAt, new Date()))).limit(1);
...
export async function getGuestToken(create = false) {
  ...
  jar.set("sufra_guest", token, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/", maxAge: 86400 * 7 });
  return token;
}
...
export function isSameOrigin(request: Request) {
  ...
  try {
    ...
    return host === request.headers.get("host") || host === request.headers.get("x-forwarded-host") || host === new URL(request.url).host;
  } catch { return false; }
}
